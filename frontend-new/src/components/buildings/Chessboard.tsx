...
const groupProperties = (properties: Property[]) => {
  return properties.reduce((acc, prop) => {
    ...
    return acc;
  }, {} as Record<string, Record<number, Property[]>>);
};
...
export default function Chessboard({ properties, onCellClick }: ChessboardProps) {
  ...
  const entrances = Object.keys(propertiesByEntrance).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));

  // Собираем все уникальные этажи по всем подъездам для выравнивания
  const allFloors = [...new Set(
    Object.values(propertiesByEntrance).flatMap(floors => Object.keys(floors).map(Number))
  )].sort((a, b) => b - a); // Сортировка по убыванию (верхние этажи сверху)
        ...
        return (
          <Paper key={entrance} sx={{ p: 1, minWidth: 300, flexShrink: 0 }} variant="outlined">
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
              {allFloors.map((floor) => {
                const floorProperties = propertiesByEntrance[entrance][floor] || [];
                return (
                  <Box key={floor} sx={{ display: 'flex', borderBottom: `1px solid ${borderColor}`, minHeight: 78 }}>
